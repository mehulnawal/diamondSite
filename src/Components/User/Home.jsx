...
const products = [
  {
    ...
    price: "$7,850",
    image: "/products/diamond1.jpg",
  },
  {
    id: 2,
    name: "Emerald Luxury Necklace",
    price: "$12,400",
    image: "/products/diamond2.jpg",
  },
  {
    id: 3,
    name: "Classic Solitaire Ring",
    price: "$6,500",
    image: "/products/diamond3.jpg",
  },
];
  ...
  return (
    <div className="w-full pt-0">
      ...
      <section className="relative h-[600px] flex items-center justify-center bg-gradient-to-br from-[#1E40AF] to-[#3B82F6] overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-2xl mx-auto">
          ...
          <p className="text-white/90 font-inter text-xl mb-8">
            Exclusive, conflict-free diamonds & fine jewelry. Crafted for those
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12 px-4">
          {features.map((f) => (
            ...
            >
              ...
              <h3 className="text-2xl font-playfair font-bold text-[#0F172A]">
                {f.title}
              </h3>
      ...
      <section className="py-16 max-w-6xl mx-auto px-4">
        <h2 className="text-3xl md:text-4xl font-playfair font-bold text-[#0F172A] mb-8 text-center">
          Featured Diamonds
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
          {products.map((product) => (
            <div
              key={product.id}
              className="bg-white rounded-xl shadow-[0_4px_6px_rgba(0,0,0,0.10)] p-6 flex flex-col gap-4 hover:shadow-[0_8px_15px_rgba(0,0,0,0.12)] hover:scale-105 transition transform duration-300 border-2 border-transparent hover:border-[#D4AF37]"
            >
              ...
              <h4 className="font-playfair text-xl font-bold text-[#0F172A]">
                {product.name}
              </h4>
      ...
      <section className="bg-[#F8FAFC] py-16">
        <div className="max-w-4xl mx-auto text-center">
          <h3 className="text-2xl md:text-3xl font-playfair font-bold text-[#0F172A] mb-8">
            Testimonials
          </h3>
          <div className="flex flex-col md:flex-row justify-center gap-8">
            {testimonials.map((t) => (
              <blockquote
                key={t.id}
                className="bg-white rounded-lg shadow p-6 italic text-[#94A3B8] font-light max-w-sm mx-auto"
              >
                "{t.quote}"
                <footer className="mt-4 text-[#1E40AF] font-montserrat font-semibold">
                  - {t.name}
                </footer>
              </blockquote>
            ))}
